...
class PropulsionSystem:
    ...
    def calculate_efficiency(self, delta_v):
        ...
        """
        # Step 1: Use simulate_fourier_diffusion to update the tank temperature over time
        # This function simulates heat diffusion and updates self.tank_temperature
        self.simulate_fourier_diffusion(time_step=10, total_time=100)  # Adjust time_step and total_time as needed

        # Step 2: Use the updated tank temperature as chamber temperature
        chamber_temperature = self.tank_temperature  # Get updated chamber temperature

        # Step 3: Update gas constant and calculate exhaust velocity based on updated tank pressure and temperature
        ...
        # Calculate exhaust velocity using the new chamber temperature and tank pressure
        ...
        # Step 4: Calculate propellant consumption for the given delta-v, using the changing mass of the satellite
        delta_v_metres = delta_v * 1e3  # Convert delta-v to m/s
        initial_mass = self.total_mass[-1]  # Last updated mass of the satellite

        # Calculate final mass after burn
        final_mass = initial_mass / np.exp(delta_v_metres / v_exhaust)
        propellant_consumed = initial_mass - final_mass

        # Update propellant mass based on consumption
        self.propellant_mass -= propellant_consumed
        self.total_mass.append(final_mass)

        # Step 5: Calculate the Kinetic Energy of Exhaust
        kinetic_energy_exhaust = 0.5 * propellant_consumed * v_exhaust**2

        # Step 6: Calculate Chemical Energy Released (Updated based on dynamic propellant amount and temperature)
        specific_heat_combustion = self.engine_thermal_properties.get('specific_heat_combustion', 4200)  # J/kg·K
        delta_temp_combustion = chamber_temperature - self.engine_thermal_properties.get('ambient_temperature', 300)  # K

        # Total chemical energy released based on propellant consumed
...
def test_propulsion_subsystem():
    ...
    # Example list of delta-v values for different mission segments (in km/s)
    delta_v_array_efficiency = [3, 5.5, 10, 8.2, 3.2, 4.5, 6, 7.5]
    efficiency_array = []
    delta_v_array_accumulated_efficiency = []

    # Loop through delta_v_array to calculate efficiencies and cumulative delta-v
    for i, delta_v in enumerate(delta_v_array_efficiency):
        # Calculate efficiency for the current delta-v
        efficiency = propulsion_system.calculate_efficiency(delta_v)
        efficiency_array.append(efficiency)  # Store the efficiency

        # Calculate cumulative delta-v for this point in the mission
        if i == 0:
            delta_v_array_accumulated_efficiency.append(delta_v)
        else:
            delta_v_array_accumulated_efficiency.append(delta_v + delta_v_array_accumulated_efficiency[i-1])

    # Plotting the results
    plt.figure(figsize=(10, 6))
    plt.plot(delta_v_array_accumulated_efficiency, efficiency_array, marker='o', linestyle='-', color='g', label='Efficiency')
    plt.xlabel('Cumulative Delta V (km/s)')
    plt.ylabel('Efficiency')
    plt.title('Cumulative Delta V vs. Propulsion Efficiency')
    plt.grid(True)
    plt.legend()
    plt.show()
